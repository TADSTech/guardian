...
export default function LoginPage() {
  ...
  const handleSignIn = (e: React.FormEvent) => {
    ...
    if (users[email] && users[email] === password) {
      localStorage.setItem("guardian_user", JSON.stringify({ email }));
            ...
            <Card>
              ...
              <form onSubmit={handleSignIn}>
                ...
                <CardFooter className="flex-col items-start gap-2">
                  ...
                  <Button type="submit" className="w-full" style={{ background: "var(--deep)" }}>Sign In</Button>
                </CardFooter>
              </form>
            </Card>
          </TabsContent>
              ...
              <form onSubmit={handleSignUp}>
                <CardContent className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <Input id="new-email" type="email" placeholder="you@example.com" value={email} onChange={(e) => setEmail(e.target.value)} required />
                  </div>
                  <div className="space-y-2">
                ...
                <CardFooter className="flex-col items-start gap-2">
                  {error && <p className="text-sm text-red-500">{error}</p>}
                  <Button type="submit" className="w-full" style={{ background: "var(--deep)" }}>Create Account</Button>
                </CardFooter>
              </form>
            </Card>
          </TabsContent>
